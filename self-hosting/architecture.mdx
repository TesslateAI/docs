...
<Tabs>
  <Tab title="Docker Mode (Development)">
    All services run on a single host machine via Docker Desktop.

    **Request routing (Traefik reverse proxy):**
  ...
  <Tab title="Kubernetes Mode (Production)">
    The cluster is organized into isolated namespaces.

    **Core namespace (`tesslate`):**
    ...
    **Per-project namespaces (`proj-{uuid}`):**
    ...
    Each project gets a dedicated namespace containing:
    - File Manager Pod (always running, handles file ops and git)
    - Dev Container Pod(s) (frontend, backend, database services)
    - PVC (10Gi EBS block storage, shared across pods)
    - NetworkPolicy (zero cross-project communication)
    ...
    **Ingress namespace (`ingress-nginx`):**
    ...
    | Component | Role |
    |-----------|------|
    | NGINX Ingress Controller | Routes `*.yourdomain.com` to project namespaces |
    | cert-manager | Wildcard SSL via Let's Encrypt + Cloudflare DNS |

    **Storage:** EBS VolumeSnapshots for persistence across sessions
  </Tab>
</Tabs>
...
<CardGroup cols={2}>
  ...
  <Card title="Developer Guide" icon="book" href="/developer/system-overview">
